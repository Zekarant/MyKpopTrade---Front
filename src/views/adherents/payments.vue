      ...
      <div class="payments-page__container">
        ...
        <div v-else class="payments-list">
          <article v-for="p in payments" :key="p._id" class="payment-card">
            ...
            <div v-if="p.refunds && p.refunds.length > 0" class="payment-card__refunds">
              <div v-for="refund in p.refunds" :key="refund._id">
                <span
                  v-if="refund.status === 'completed'"
                  class="refund-badge refund-badge--completed"
                  :data-tooltip="refund.reason || ''"
                >
                  <i class="bi bi-arrow-counterclockwise"></i> Remboursement envoyé
                </span>
                <span v-else-if="refund.status === 'pending'" class="refund-badge refund-badge--pending">
                  <i class="bi bi-hourglass-split"></i> Remboursement en cours
                </span>
                <span v-else-if="refund.status === 'failed'" class="refund-badge refund-badge--failed">
...
interface Payment {
  ...
  refunds?: Array<{ _id: string; status: 'completed' | 'pending' | 'failed'; reason?: string }>;
  shipment?: {
    carrier: string;
