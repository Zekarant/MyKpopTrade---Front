<template>
  ...
  export default defineComponent({
    ...
    setup(props) {
        ...
        // Payment configuration check (PayPal or Stripe)
        ...
        const checkPaymentConfiguration = async () => {
            try {
                ...
                const [paypalStatus, stripeStatus] = await Promise.allSettled([
                    paymentService.getPayPalConnectionStatus(),
                    paymentService.getStripeAccountStatus(),
                ]);
                paypalOAuthConnected.value = paypalStatus.status === 'fulfilled' && Boolean(paypalStatus.value.oauthConnected);
